  ...
  const renderRouteCard = (route, isSearched = false) => (
    <div key={route.id} className={`border-2 rounded-lg p-5 hover:shadow-lg transition-all ${isSearched ? 'border-primary-500 border-4 bg-primary-50' : selectedRoute?.id === route.id ? 'border-primary-500' : 'border-gray-200'}`}>
      ...
      <div className="bg-primary-50 p-3 rounded-lg mb-3">
        ...
        <div className="flex flex-wrap gap-2">{route.highlights.map((highlight, idx) => (<span key={idx} className="px-2 py-1 bg-white text-primary-700 rounded text-xs font-medium">✓ {highlight}</span>))}</div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card bg-gradient-to-r from-primary-600 to-primary-700 text-white">
        ...
        <button onClick={handleSearchRoute} disabled={isLoading} className="w-full bg-white text-primary-600 py-3 rounded-lg hover:bg-primary-50 font-semibold flex items-center justify-center disabled:opacity-50">
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {activeRouteData.nearbyHelpPoints.map((point, index) => (<div key={index} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow"><div className="flex items-center justify-between mb-2"><span className={`px-2 py-1 rounded text-xs font-semibold ${point.type === 'Police' ? 'bg-primary-100 text-primary-700' : point.type === 'Medical' ? 'bg-danger-100 text-danger-700' : 'bg-success-100 text-success-700'}`}>{point.type}</span><span className="text-sm font-bold text-gray-600">{point.distance}</span></div><p className="font-semibold text-gray-900">{point.name}</p></div>))}
        </div>
      </div>
